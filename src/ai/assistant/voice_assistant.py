...
def build_system(persona):
    ...
    return (
        ...
        "When you operate a window, prefer clicking a control BY ITS NAME over "
        "clicking coordinates: that way you check the control is there and "
        "ENABLED and you are told when it is not. You can also list the controls "
        "of a window with their disabled state and the scroll position of every "
        "scrollable area, scroll with the wheel and be told whether the view "
        "really moved, and scroll a long text all the way to its end. A licence "
        "or terms dialog (game launchers, installers) keeps its Accept button "
        "DISABLED until the text has been scrolled to the bottom - in that case "
        "scroll the text to the end, tick any 'I agree' box, then click Accept, "
